...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function addFieldValidations() {
        ...
        fieldsToValidate.forEach(fieldId => {
            ...
            if (field) {
                field.addEventListener('input', () => {
                    ...
                    if (!/^\d+$/.test(field.value)) { // Verificar si es numérico
                        showErrorAlert(`El campo ${fieldId} debe contener solo números.`);
                        field.value = "";  // Limpiar el campo si no es numérico.
                    }
                });
            }
        });

        // Validación adicional para celular (9 dígitos)
        ...
        if (celularField) {
            celularField.addEventListener('blur', () => { // 'blur' cuando se quita el foco
                if (celularField.value.length !== 9) {
                    ...
                    celularField.value = ''; // Limpia el campo si no tiene 9 dígitos.
                }
            })
        }
        ...
        const nameFields = [
            ...
            'representante_nombres', 'representante_apellido_paterno', 'representante_apellido_materno',
            'razon_social'
        ];
    ...
    function showErrorAlert(message) {
        alert(message); // Reemplaza esto con una mejor forma de mostrar errores (ej., un div)
    }
});